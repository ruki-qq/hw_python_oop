...
def test_Training():
    ...
    for name, value in {
        ...
        'M_IN_KM': ValueInfo(
            1000, ' для перевода значений из метров в километры'
        ),
    }.items():
    ...
    assert MIN_IN_H in homework.Training.__dict__.values(), (
        'У класса `Training` должна быть константа для перевода часов в '
...
def test_Swimming():
    ...
    for value_info in {
        ...
        assert value_info.value in Swimming_attr_values, (
            'У класса `Swimming` должна быть константа'
...
def test_SportsWalking():
    ...
    for value_info in {
        ...
        'KMH_IN_MSEC': ValueInfo(
            0.278, ' для перевода значений из км/ч в м/с'
        ),
        'CM_IN_M': ValueInfo(
            100, ' для перевода значений из сантиметров в метры'
        ),
    }.values():
...
def test_Running():
    ...
    for value_info in {
        ...
        'CALORIES_MEAN_SPEED_SHIFT': ValueInfo(
            1.79, ' для сдвига средней скорости'
        ),
    }.values():
        assert value_info.value in Running_attr_values, (
            'У класса `Running` должна быть константа'
            f'{value_info.description} со значением `{value_info.value}`.'
        )
